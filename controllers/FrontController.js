...
class FrontController {
    ...
    static verifyLogin = async (req, res) => {
        try {
            ...
            // Now we check email is registered or not
            ...
            if (!user) {
                ...
                if (isMatch) {
                    if (user.role == 'admin') {
                        // token
                        const token = jwt.sign({ ID: user.id }, 'duhoehkj637484hfj')
                        // console.log(token)
    ...
    //                         let token = jwt.sign({ ID: user.id }, "duhoehkj637484hfj");
    ...
    static verifyMail = async (req, res) => {
        try {
            ...
            if (updateinfo) {
                let token = jwt.sign({ ID: updateinfo.id }, 'duhoehkj637484hfj')
                //console.log(token)middleware
    ...
    static sendEmail = async (name, email, token) => {
        ...
        let info = await transporter.sendMail({
            ...
            html: "<p>Hii " +
                name +
                ',Please click here to <a href="https://collage-portal-ehso.onrender.com/reset-password?token=' +
                token +
